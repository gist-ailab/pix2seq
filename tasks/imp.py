...
class TaskObjectClassification(task_lib.Task):
  """Object classification task with coco metric evaluation."""

  def __init__(self, config: ml_collections.ConfigDict):
    super().__init__(config)

    if config.task.get('max_seq_len', 'auto') == 'auto':
      self.config.task.max_seq_len = config.task.max_instances_per_image
  ...
  def preprocess_batched(self, batched_examples, training):
    ...
    ret = build_response_seq_from_label(labels['label'], mconfig.coord_vocab_shift, class_label_corruption=config.class_label_corruption)
  ...
  def infer(self, model, preprocessed_outputs):
    ...
    pred_seq, logits, _ = model.infer(
        ...
        max_seq_len=(config.max_instances_per_image_test + 1),
  ...
  def postprocess_tpu(self, batched_examples, pred_seq, logits, training=False):
    ...
    pred_classes, scores = task_utils.decode_object_seq_to_cls(logits, pred_seq, mconfig.coord_vocab_shift)

    # Compute coordinate scaling from [0., 1.] to actual pixels in orig image.
    gt_classes = labels['label']
    return (images, image_ids, pred_classes, scores, gt_classes)
  ...
  def postprocess_cpu(self, outputs, train_step,
    ...
    (images, image_ids, pred_classes, scores, gt_classes) = new_outputs
...
def build_annotations(image_ids, category_ids, scores, counter) -> List[Dict[str, Any]]:
  """Builds annotations."""
  annotations = []
